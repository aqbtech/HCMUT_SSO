...
public class AuthenticationService {
	...
	public AuthenticationResponse authenticate(AuthenticationRequest request) {
		...
		var user = userRepository.findByUsername(request.getUsername())
					.orElseThrow(() -> new ServerException("Unauthenticated", HttpStatus.UNAUTHORIZED));

		boolean authenticated = passwordEncoder.matches(request.getPassword(), user.getPassword());

		if (!authenticated) throw new ServerException("Unauthenticated", HttpStatus.UNAUTHORIZED);
	...
	private SignedJWT verifyToken(String token, boolean isRefresh) throws JOSEException, ParseException {
		...
		if (!(verified && expiryTime.after(new Date())))
			throw new ServerException("Unauthenticated", HttpStatus.UNAUTHORIZED);
